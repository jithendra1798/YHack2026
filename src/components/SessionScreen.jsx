...
export default function SessionScreen({ context, mode, onBack }) {
  const {
    transcript,
    transcriptVersion,
    isListening,
  ...
  return (
    <div className="h-screen flex flex-col bg-[#06060b] overflow-hidden relative">
      ...
      <div className="relative z-10 flex flex-col h-full">
        <Header mode={mode} isListening={isListening} onBack={handleBack} />

        {!isSupported && mode === 'live' && (
        ...
        {micError && mode === 'live' && (
          <div className="px-6 py-3 bg-[#ff6b35]/[0.08] border-b border-[#ff6b35]/20">
            <div className="flex items-center justify-center gap-3">
              ...
              <p className="text-[#ff6b35] text-[12px] font-medium">
                {micError}
              </p>
              {permissionDenied && (
                <button
                  onClick={handleRetryMic}
                  className="ml-2 px-3 py-1 rounded-md text-[11px] font-bold bg-[#ff6b35]/15 text-[#ff6b35] border border-[#ff6b35]/25 hover:bg-[#ff6b35]/25 transition-all"
                >
                  Retry
                </button>
              )}
        ...
        {analysisError && (
          <div className="px-6 py-2 bg-[#ff3b5c]/[0.06] border-b border-[#ff3b5c]/15">
            <p className="text-[#ff3b5c]/70 text-[11px] text-center font-mono">
        ...
        <div className="flex-1 flex min-h-0">
          <div className="w-[40%] border-r border-white/[0.04] flex flex-col relative">
            ...
            <div className="flex items-center gap-2.5 px-4 py-2.5 border-b border-white/[0.04]">
              ...
              {!isListening && mode === 'live' && !micError && (
                <button
                  onClick={startListening}
                  className="ml-auto text-[9px] font-bold px-2 py-1 rounded bg-[#00f0ff]/10 text-[#00f0ff]/60 border border-[#00f0ff]/15 hover:bg-[#00f0ff]/20 transition-all"
                >
                  Start Mic
                </button>
              )}
